...
class mixedresnetnetwork(nn.Module):
    ...
    def forward(self, data):
        split = torch.split(data, [(data.size(2)-5), 5], dim=2) # Tensor data is combined image and numeric feature data
        images = split[0].view(split[0].shape[0], 3, 224, 224)
        descriptors = split[1]
        x = self.image_features_(images) # Performs feature extraction on image data
        ...
        z = torch.cat((x.view(x.size(0), -1), y.view(y.size(0), -1)), dim=1) # Concatenate image and feature data post-feature extraction
